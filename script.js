...
function prevPhase() {
    if(currentPhaseIndex > 0) {
        ...
        location.reload(); // 最初の問題で戻るとタイトルへ
    }
}
...
function setupScalePhase() { const a = document.createElement('div'); a.style.marginTop = '20px'; const s = document.createElement('div'); s.textContent = '🍄'; s.style.fontSize = '5rem'; let sz = 5; const b1 = document.createElement('span'); b1.className = 'fa-solid fa-wine-bottle scale-btn'; b1.onclick = () => { sz--; s.style.fontSize = sz+'rem'; tracker.scaleShrink++; updateLog(`縮小エネルギー[${tracker.scaleShrink}]`); }; const b2 = document.createElement('span'); b2.className = 'fa-solid fa-cookie-bite scale-btn'; b2.onclick = () => { sz++; s.style.fontSize = sz+'rem'; tracker.scaleGrow++; updateLog(`拡大エネルギー[${tracker.scaleGrow}]`); }; const b = document.createElement('button'); b.className = 'btn'; b.textContent = '決定'; b.onclick = nextPhase; a.append(b1, s, b2); mainArea.appendChild(a); mainArea.appendChild(b); }
...
function setupSortPhase() {
    ...
    btn.onclick = () => {
        ...
        if (pattern === 'black-red-black-red' || pattern === 'red-black-red-black') {
            tracker.isAlternating = true;
            updateLog("赤黒交互の秩序を検出！(Ti)");
        }
        nextPhase();
    };
...
function setupTeObstaclePhase() { const a = document.createElement('div'); a.className = 'play-area'; const t = document.createElement('div'); t.textContent = '🗑️'; t.style.cssText = "position:absolute; right:5%; bottom:5%; font-size:4rem; z-index:1;"; a.appendChild(t); const sT = Date.now(); let rc = 0; for(let i=0; i<3; i++) { const r = document.createElement('div'); r.textContent = '🪨'; r.className = 'item'; r.style.left = (20+Math.random()*50)+'%'; r.style.top = (20+Math.random()*50)+'%'; r.style.zIndex = '2'; makeDraggable(r, a, () => updateLog("排除中..."), () => { if(checkCollision(r, t)) { r.style.display = 'none'; rc++; updateLog(`排除: ${rc}/3`); if(rc === 3) { tracker.obstacleTime = Date.now() - sT; activeIntervals.push(setTimeout(nextPhase, 600)); } } }); a.appendChild(r); } const b = document.createElement('button'); b.className = 'btn'; b.textContent = '諦める'; b.onclick = () => { if(tracker.obstacleTime === 0) tracker.obstacleTime = Date.now() - sT; nextPhase(); }; mainArea.appendChild(a); mainArea.appendChild(b); }
...
function setupSeAttackPhase() {
    ...
    const iv = setInterval(() => {
        ...
        if (p > 85) { clearInterval(iv); updateLog("押し切られた..."); nextPhase(); }
    }, 100);
    activeIntervals.push(iv);
    const b = document.createElement('button'); b.className = 'btn'; b.innerHTML = '⚔️ 押し返す！';
    b.style.cssText = "position:absolute; bottom:10px; left:50%; transform:translateX(-50%); z-index:10; padding:15px 30px;";
    b.onclick = () => {
        p -= 7; tracker.attackClicks++; s.style.right = p + '%';
        if (p < 0) {
            clearInterval(iv);
            tracker.attackTime = Date.now() - startAtk;
...
function setupSiCushionPhase() { const a = document.createElement('div'); a.className = 'play-area'; const m = document.createElement('div'); m.textContent = '🍄'; m.style.cssText = "position:absolute; left:40%; top:50%; font-size:5rem;"; const c = document.createElement('div'); c.textContent = '🍪'; c.className = 'item'; makeDraggable(c, a, () => { tracker.siMicroMovements++; updateLog(`微調整[${tracker.siMicroMovements}回]`); }); a.append(m, c); const b = document.createElement('button'); b.className = 'btn'; b.textContent = '完了'; b.onclick = nextPhase; mainArea.appendChild(a); mainArea.appendChild(b); }

function setupRosesPhase() { const a = document.createElement('div'); a.style.marginTop = '20px'; for(let i=0; i<5; i++){ const r = document.createElement('div'); r.textContent = '🌹'; r.className = 'rose'; r.onclick = () => { if(!r.classList.contains('painted')){ r.classList.add('painted'); tracker.rosesPainted++; updateLog(`彩り[${tracker.rosesPainted}本]`); }}; a.appendChild(r); } const b = document.createElement('button'); b.className = 'btn'; b.textContent = '塗り終わり'; b.onclick = nextPhase; mainArea.appendChild(a); mainArea.appendChild(b); }
...
function showResult() {
    ...
    mainArea.innerHTML = `
        <div id="result-capture-area" style="background:var(--card-bg); padding:15px; border-radius:10px; border:3px solid var(--text-dark);">
            ...
            <div style="background:rgba(255,255,255,setupDarlingPhase0.8); padding:15px; border-radius:12px; margin:15px 0; border:2px solid var(--text-dark); text-align:left; line-height:1.6; font-size:0.95rem; white-space:pre-wrap;">${typeData.desc}</div>
